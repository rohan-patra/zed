    ...
    fn serialize_workspace_internal(&self, window: &mut Window, cx: &mut App) -> Task<()> {
        ...
        match self.workspace_location(cx) {
            WorkspaceLocation::Location(location, paths) => {
                ...
                let is_empty_local_workspace =
                    paths.is_empty() && location == SerializedWorkspaceLocation::Local;
                let default_docks = is_empty_local_workspace.then(|| docks.clone());
                let has_open_items = self.panes.iter().any(|pane| pane.read(cx).items_len() > 0);
                // A project-less window with nothing open uses `default_dock_state`;
                // letting it also write the shared layout would clobber real projects' docks.
                let global_docks = (global_dock_layout_enabled(cx)
                    && (!is_empty_local_workspace || has_open_items))
                    .then(|| docks.clone());
